...
#' @examples
#' quarters <- qyear_gam(raster_grid, q_yr_combo, catch_df) 
#' @export
qyear_gam <- function(raster_grid, q_yr_combo, catch_df){
  #Run the analysis for each Quarter-Year combination
  gam_raster <- apply(q_yr_combo,
                       ...
                       function(x) {
                         ...
                         calculate_gam(raster_grid, catch_sf) 
                       })
  #Turn the list of rasters into a spatraster with different layers
  gam_raster <- terra::rast(gam_raster)
  names(gam_raster) <- as.character(q_yr_combo$Year)
  #We split the raster layers per quarter in q_yr_combo
  split_list <- q_yr_combo$quarter
  raster_split <- terra::split(gam_raster, split_list)
