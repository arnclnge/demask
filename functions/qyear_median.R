...
#' @export
qyear_median <- function(raster_grid, q_yr_combo, catch_df){
  #Run the analysis for each Quarter-Year combination
  median_raster <- apply(q_yr_combo,
                       ...
                       function(x) {
                         ...
                         calculate_median(raster_grid, catch_sf) 
                       })
  #Turn the list of rasters into a spatraster with different layers
  median_raster <- terra::rast(median_raster)
  names(median_raster) <- as.character(q_yr_combo$Year)
  #We split the raster layers per quarter in q_yr_combo
  split_list <- q_yr_combo$quarter
  raster_split <- terra::split(median_raster, split_list)
