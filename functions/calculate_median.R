...
#' @examples
#' mean_raster <- calculate_median(r, catch_sf) 
#' @export
calculate_median <- function(raster_grid, catch_sf, buffer = 65000){
  ...
  median_catch <- sapply(contains_list, function(idx) {
    if (length(idx) == 0) {
      return(NA)  # no points in this buffer
    } else {
      median(catch_sf$Catch[idx], na.rm = TRUE)
    }
  })
  
  #Each buffer now contains the mean catch value
  center_buffer$median_catch <- median_catch
  
  #Give each center of the grid cells this value for easier rasterizing
  center_grid$median_catch <- median_catch
  
  #Rasterize mean catchweight
  r_median <- rasterize(x = center_grid,
